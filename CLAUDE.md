...
本專案使用 **Python 型別註解 (Type Annotations)** 和 **mypy 靜態型別檢查**，確保程式碼的型別安全性、可維護性和可讀性。
...
# 檢查特定目錄
uv run mypy src/core/ --config-file pyproject.toml
```
...
**修改程式碼前**：
1. 閱讀 `docs/type-annotation-guide.md` - 完整的型別註解指南
2. 參考現有程式碼的型別註解模式
3. 了解專案使用的型別別名（`src/core/type_aliases.py`）
...
# ✅ 好的做法：使用型別別名
from src.core.type_aliases import RecordList, DownloadResult

def get_records(start_date: str) -> RecordList:
    """獲取記錄清單"""
    return []
...
```bash
# 立即執行型別檢查（重要！）
uv run mypy <修改的檔案> --config-file pyproject.toml
...
- 每次 commit 前自動執行型別檢查
- 確保提交的程式碼通過型別驗證
- 安裝：`pre-commit install`（已在安裝腳本中自動執行）

**CI/CD 整合**：
- GitHub Actions 自動執行型別檢查
- PR 必須通過型別檢查才能合併
- 工作流程：`.github/workflows/type-check.yml`

#### 型別註解最佳實踐

詳細的型別註解指南請參考：**[docs/type-annotation-guide.md](docs/type-annotation-guide.md)**

指南包含：
- 基本型別註解語法
- WebDriver、日期、回調等專案特定型別模式
- Do's and Don'ts
- 常見問題解決方案
- IDE 配置建議
...
- **整體覆蓋率**: 81.9% (目標: 90%)
- **核心模組**: 100% 覆蓋（0 errors）
- **優秀模組**: `constants.py`, `type_aliases.py`, `smart_wait.py`
- **改進中**: `freight_scraper.py`, `diagnostic_manager.py`

完整報告：執行 `./scripts/type_check.sh --report` 查看
