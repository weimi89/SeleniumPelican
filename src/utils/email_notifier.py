...
class EmailNotifier:
    ...
    def __init__(
        ...
        port_value = port if port is not None else os.getenv("MAIL_PORT")
        self.port = int(self._clean_value(str(port_value) if port_value else "587"))
    ...
    def send_message(
        ...
        try:
            ...
            if self.encryption == "ssl":
                server = smtplib.SMTP_SSL(str(self.host), self.port, timeout=30)
            else:
                smtp_server = smtplib.SMTP(str(self.host), self.port, timeout=30)
                if self.encryption == "tls":
                    smtp_server.starttls()
                server = smtp_server

            server.login(str(self.username), str(self.password))
            server.sendmail(str(self.from_address), self.to_addresses, msg.as_string())
            server.quit()
...
📋 處理步驟：
----------------------------------------
1. 登入宅配通 WEDI 系統 (http://wedi.e-can.com.tw/)
2. 依照系統提示更新密碼
...
""".strip()
    ...
    def send_execution_summary(
        ...
        body_parts = [
            ...
            "",
            "========================================",
            f"台灣宅配通自動化工具 • {total_execution_minutes:.2f} 分鐘",
            "========================================",
        ]
