...
class BaseScraper:
            ...
            if matches:
                for match in matches:
                    ...
                    if match.isdigit() and 1900 <= int(match) <= 2100:
                        continue
                    self.logger.log_operation_success(f"從頁面找到可能的驗證碼: {match}")
                    return match
    ...
    def navigate_to_query(self):
        ...
        try:
            # 點擊查詢作業選單
            query_menu = self.wait.until(EC.element_to_be_clickable((By.LINK_TEXT, "查詢作業")))
            query_menu.click()
            time.sleep(2)
            self.logger.log_operation_success("點擊查詢作業選單")

            # 點擊查件頁面
            query_page = self.wait.until(EC.element_to_be_clickable((By.LINK_TEXT, "查件頁面")))
            query_page.click()
            time.sleep(5)  # 等待頁面載入
            ...
            self.logger.log_operation_success("切換到 datamain iframe", status="ready_for_data")
