...
一個使用 Python + Selenium 建立的現代化自動化工具套件，專門用於自動登入 WEDI（台灣宅配通）系統並下載各種資料。支援代收貨款匯款明細、運費(月結)結帳資料查詢，以及運費未請款明細下載。
...
🔔 **即時通知**: 支援 Discord Webhook 和 Email SMTP 通知
...
│       └── email_notifier.py     # Email SMTP 通知模組
├── tests/                        # 測試框架 (pytest)
...
PAYMENT_DOWNLOAD_WORK_DIR="downloads/代收貨款"         # 代收貨款匯款明細下載路徑
UNPAID_DOWNLOAD_WORK_DIR="downloads/運費未請款"        # 運費未請款明細下載路徑
FREIGHT_DOWNLOAD_WORK_DIR="downloads/運費發票"         # 運費發票明細下載路徑

# 已下載檔案檢查目錄（選用，預設使用對應的 WORK_DIR）
PAYMENT_DOWNLOAD_OK_DIR="downloads/代收貨款/ok"       # 代收貨款已下載檔案檢查目錄
UNPAID_DOWNLOAD_OK_DIR="downloads/運費未請款/ok"      # 運費未請款已下載檔案檢查目錄
FREIGHT_DOWNLOAD_OK_DIR="downloads/運費發票/ok"       # 運費發票已下載檔案檢查目錄
```
...
執行完成後可自動發送通知，支援 Discord Webhook 和 Email SMTP 兩種方式。

### Discord 通知

在 `.env` 中設定 Discord Webhook URL：
...
**通知內容**：
- 執行報告（成功/失敗帳號數、下載檔案清單）
- 密碼安全警告（需要更新密碼的帳號）

### Email 通知

在 `.env` 中設定 SMTP 伺服器資訊：
...
- `MAIL_PORT` - SMTP 埠號（587 for TLS, 465 for SSL）
- `MAIL_ENCRYPTION` - 加密方式（`tls`、`ssl` 或 `none`）
- `MAIL_TO_ADDRESS` - 支援多個收件人，用逗號分隔

**通知內容**：
- 執行摘要（帳號統計、下載統計）
- 密碼更新提醒

> **提示**：Discord 和 Email 通知可同時啟用，也可只設定其中一種。未設定的通知方式會自動跳過。
...
# 下載路徑設定（可選）
PAYMENT_DOWNLOAD_WORK_DIR="downloads/代收貨款"         # 代收貨款匯款明細
UNPAID_DOWNLOAD_WORK_DIR="downloads/運費未請款"        # 運費未請款明細
FREIGHT_DOWNLOAD_WORK_DIR="downloads/運費發票"         # 運費發票明細
```
...
**Q: 我想讓檔案自動分類怎麼辦？**
A: 在 `.env` 設定不同的 `*_DOWNLOAD_DIR`，例如 `PAYMENT_DOWNLOAD_WORK_DIR="downloads/代收貨款"`。
