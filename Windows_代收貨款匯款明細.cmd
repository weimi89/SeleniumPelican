...
if not exist "scripts\run_payment.ps1" (
    echo ❌ 錯誤：找不到 scripts\run_payment.ps1 檔案
    echo 📁 當前目錄：%CD%
    pause
    exit /b 1
)
...
if %errorlevel%==0 (
    ...
    if %errorlevel%==0 (
        ...
        wt -w 0 -p "PowerShell" pwsh -NoExit -ExecutionPolicy Bypass -WorkingDirectory "%CD%" -File "%CD%\scripts\run_payment.ps1" %*
        goto :end
    )
)
...
if %errorlevel%==0 (
    ...
    start "WEDI 代收貨款查詢" pwsh -NoExit -ExecutionPolicy Bypass -WorkingDirectory "%CD%" -File "%CD%\scripts\run_payment.ps1" %*
    goto :end
)
